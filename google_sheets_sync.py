...
def parse_date(value):
    ...
    try:
        ...
        if isinstance(value, (int, float)):
            # Handle Google Sheets/Excel serial numbers (1899-12-30 base)
        ...
        if isinstance(value, str):
            ...
            if not cleaned:
                return None
            # Handle ISO format with optional trailing Z
...
def convert_ingredient_id_to_string(session: Session, ingredient_id: Optional[Any]) -> Optional[str]:
    ...
    if ingredient_id is None:
        return None
    # If it's already a string, return it (strip spaces)
...
def sync_starters_from_sheet(service, spreadsheet_id: str, session: Session):
    ...
    try:
        ...
        for record in records:
            ...
            water_type_id = convert_ingredient_id_to_string(session, normalize_sheet_identifier(record.get('water_type')))
            kake_id = convert_ingredient_id_to_string(session, normalize_sheet_identifier(record.get('Kake')))
            koji_id = convert_ingredient_id_to_string(session, normalize_sheet_identifier(record.get('Koji')))
            yeast_id = convert_ingredient_id_to_string(session, normalize_sheet_identifier(record.get('yeast')))
            lactic_acid_value = record.get('lactic_acid_g')
            if lactic_acid_value is None:
                lactic_acid_value = record.get('lactic_acid')
            mgso4_value = record.get('MgSO4_g')
            if mgso4_value is None:
                mgso4_value = record.get('MgSO4')
            kcl_value = record.get('KCl_g')
            if kcl_value is None:
                kcl_value = record.get('KCl')
            ...
            starter = Starter(
                ...
                lactic_acid=parse_float(lactic_acid_value),
                MgSO4=parse_float(mgso4_value),
                KCl=parse_float(kcl_value),
    ...
    try:
        ...
        for record in records:
            ...
            recipe = Recipe(
                ...
                pasteurization_notes=record.get('pasteruization_notes') or record.get('pasteurization_notes'),
...
def sync_ingredients_to_sheet(service, spreadsheet_id: str, session: Session):
    """Write all ingredient records from the database to the Google Sheet"""
    try:
        statement = select(Ingredient)
        ...
        for ing in db_ingredients:
            rows.append({
                'ingredientID': ing.ingredientID or '',
        ...
        write_sheet_data(service, spreadsheet_id, "Ingredients", headers, rows)
        print(f"Wrote {len(rows)} ingredients to sheet")
    except Exception as e:
        print(f"Error syncing ingredients to sheet: {e}")
        raise

def sync_starters_to_sheet(service, spreadsheet_id: str, session: Session):
    """Write all starter records from the database to the Google Sheet"""
    try:
        statement = select(Starter)
        db_starters = session.exec(statement).all()
        headers = ['Date', 'StarterBatch', 'BatchID', 'Amt_Kake', 'Amt_Koji', 'Amt_water',
                   'water_type', 'Kake', 'Koji', 'yeast', 'lactic_acid_g', 'MgSO4_g', 'KCl_g', 'temp_C']
        rows = []
        for st in db_starters:
            rows.append({
...
def sync_recipes_to_sheet(service, spreadsheet_id: str, session: Session):
    """Write all recipe records from the database to the Google Sheet"""
    try:
        statement = select(Recipe)
        db_recipes = session.exec(statement).all()
        headers = [
            'start_date', 'pouch_date', 'batchID', 'batch', 'style',
            'kake', 'koji', 'yeast', 'starter', 'water_type',
            ...
            'final_measured_temp_C', 'final_measured_gravity', 'final_measured_Brix_%',
            'final_gravity', 'ABV_%', 'SMV', 'final_water_addition_mL',
            'clarified', 'pasteurized', 'pasteruization_notes', 'finishing_additions'
        ]
        rows = []
        for recipe in db_recipes:
            rows.append({
                ...
                'pasteruization_notes': recipe.pasteurization_notes or '',
                'finishing_additions': recipe.finishing_additions or ''
            })
        write_sheet_data(service, spreadsheet_id, "Recipe", headers, rows)
        print(f"Wrote {len(rows)} recipes to sheet")
    except Exception as e:
        print(f"Error syncing recipes to sheet: {e}")
        raise

def sync_publishnotes_to_sheet(service, spreadsheet_id: str, session: Session):
    """Write all publish note records from the database to the Google Sheet"""
    try:
        ...
        write_sheet_data(service, spreadsheet_id, "PublishNotes", headers, rows)
        print(f"Wrote {len(rows)} publish notes to sheet")
    except Exception as e:
...
def sync_to_google_sheets(spreadsheet_id: str, creds_path: Optional[str] = None):
    ...
    try:
        ...
        session.close()
def normalize_sheet_identifier(value: Optional[Any]) -> Optional[Any]:
    """Normalize raw sheet values by stripping whitespace from strings and treating blanks as None"""
    if value is None:
        return None
    if isinstance(value, str):
        cleaned = value.strip()
